  ...
  <div class="form-group select-page-size-wrap ">
    <label>Filas por p&aacute;gina
      <select class="form-control selectpicker show-tick" title="Rows on page" selectpicker
              ng-model="requisitions_table.tamanioTablaTodasLasRequisiciones" ng-options="i for i in [5,10,15,20,25]">
      </select>
    </label>
  </div>
  <table class="table table-hover table-condensed" st-table="requisitions_table.todasLasRequisiciones" st-safe-src="requisitions_table.requisiciones">
    <thead>
    <tr class="sortable ">
    ...
    <tbody>
    <tr ng-repeat="sl in requisitions_table.todasLasRequisiciones" ng-click="showRequisitionDetail(sl.id)" ng-class="{'primary': ((sl.alert == 1) && (sl.status == 4))}" class="row-table">
      ...
      <td>@{{sl.empresa}}</td>
      <td>@{{sl.estado}}</td>
        <td><i ng-if="((sl.alert == 1) && (sl.status == 4))" class="fa fa fa-bell"></i></td>
    </tr>
    </tbody>
    <tfoot>
    <tr>
      <td colspan="6" class="text-center">
        <div st-pagination="" st-items-by-page="requisitions_table.tamanioTablaTodasLasRequisiciones" st-displayed-pages="5"></div>
      </td>
    </tr>
    </tfoot>
  </table>
    
    <div ng-if="(requisitions_table.todasLasRequisiciones.length == 0) && todasLasRequisiciones_loaded" class="alert bg-warning text-center">
        <h4><i class="fa fa-exclamation-triangle"></i> Sin datos para mostrar</h4>
    </div>
    <div ng-if="(requisitions_table.todasLasRequisiciones.length == 0) && !todasLasRequisiciones_loaded" class="alert bg-info text-center">
