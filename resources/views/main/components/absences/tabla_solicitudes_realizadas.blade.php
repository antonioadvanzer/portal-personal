...
<div class="horizontal-scroll">
  ...
  <table class="table table-hover table-condensed" st-table="absences_table.solicitudesPropias" st-safe-src="absences_table.propias">
    ...
    <tbody>
    <tr ng-repeat="sl in absences_table.solicitudesPropias" ng-click="showOwnAbsence(sl.id)" ng-class="{'primary': ((sl.alerta == 1) && ((sl.status == 1) || (sl.status == 4) || (sl.status == 5)))}" class="row-table">
    ...
    <div ng-if="(absences_table.solicitudesPropias.length == 0) && solicitudesPropias_loaded" class="alert bg-warning text-center">
        <h4><i class="fa fa-exclamation-triangle"></i> Sin datos para mostrar</h4>
    </div>
    <div ng-if="(absences_table.solicitudesPropias.length == 0) && !solicitudesPropias_loaded" class="alert bg-info text-center">
