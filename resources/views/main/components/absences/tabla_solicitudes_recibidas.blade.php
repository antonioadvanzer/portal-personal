<div class="horizontal-scroll">
  ...
  <table class="table table-hover table-condensed" st-table="absences_table.solicitudesRecibidas" st-safe-src="absences_table.recibidas">
    ...
    <tbody>
    <tr ng-repeat="sl in absences_table.solicitudesRecibidas" ng-click="showAbsenceReceived(sl.id)" ng-class="{'primary': (sl.alerta == 1)}" class="row-table">
    ...
    <div ng-if="(absences_table.solicitudesRecibidas.length == 0) && solicitudesRecibidas_loaded" class="alert bg-warning text-center">
        <h4><i class="fa fa-exclamation-triangle"></i> Sin datos para mostrar</h4>
    </div>
    <div ng-if="(absences_table.solicitudesRecibidas.length == 0) && !solicitudesRecibidas_loaded" class="alert bg-info text-center">
