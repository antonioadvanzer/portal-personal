<div class="row-fluid">
  <div class="col-xlg-3 col-lg-12 col-sm-6 col-sm-6">
    <div ba-panel ba-panel-class="banner-column-panel">
      <div class="banner">
        <div class="large-banner-wrapper">
          <img ng-src="@{{::( 'app/evaluaciones/evaluacionmeta4.jpg' | appImage )}}" alt=""/>
        </div>
        <div class="banner-text-wrapper">
          <div class="banner-text">
            <h1>Evaluaci&oacute;n de Desempeño</h1>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<div class="typography-document-samples row-fluid">
  
  <div class="col-xlg-3 col-lg-12  col-md-6 col-sm-6">
    <div ba-panel ba-panel-class="with-scroll more-text-widget" ba-panel-title="Tipos de Evaluaci&oacute;n">
    <div class="section-block light-text" align="left">
        <p>Recuerda que todos participamos en el proceso de evaluaci&oacute;n anual dado que existen 4 tipos de evaluaci&oacute;n: Anual, Por Proyectos, Evaluaci&oacute; 360º y Autoevaluaci&oacute;n.</p>
      </div>
      <div class="section-block regular-text" align="left">
        <p><b>Anual: </b> En esta evaluaci&oacute;n se revisan y eval&uacute;n las competencias laborales y responsabilidades funcionales de acuerdo a tu posici&oacute;n y &aacute;rea. Se realiza por parte de nuestros Jefes Directos.</p><br>
        <p><b>Por Proyecto: </b> Si durante el año estuvimos asignados durante mas de 3 meses a uno o mas proyectos, los lideres de esos proyectos, evaluar&aacute;n de manera gen&eacute;rica tus competencias laborales.</p><br>
        <p><b>Evaluaci&oacute; 360º: </b> Se eval&uacute;an de manera gen&eacute;rica competencias laborales. Esta aplica &uacute;nicamente para Gerentes, Gerentes Sr y Directores. Demanera aletoria, a algunos se nos pedir realizar esta evaluaci&oacute;n siempre y cuando hayamos colaborado con alguien que tenga esa posici&oacute;n. Esta evaluaci&oacute;n es confidencial y el evaluado solo conocer&aacute; el resultado final y no quien(es) lo evaluaron. Si te toca evaluar a un superior o compañero de trabajo, debes de ser muy objetivo al responder. Se aplican entre 3 y 5 evaluaciones 360º por cada colaborador en las posiciones mencionadas.</p><br>
        <p><b>Autoevaluaci&oacute;n: </b> El c&oacute;mo te ves a tu mismo tambi&eacute;n es importante por lo que todos deberemos realizar una autoevaluaci&oacute;n de nuestras competencias laborales.</p>
      </div>
    </div>
  </div>

  <div class="col-xlg-3 col-lg-12  col-md-6 col-sm-6">
    <div ba-panel ba-panel-class="with-scroll more-text-widget" ba-panel-title="Escala de Evaluaci&oacute;n">
      <div class="section-block light-text" align="left">
        <p>Como evaluador anual, de proyecto, 360º o al elaborar tu autoevaluaci&oacute;n deber&aacute;s calificar objetivamente en una escala de 1 al 5 cada reactivo que se te presente.</p>
        <p>Veamos con un ejemplo lo que significa la escala:</p>
      </div>
      <div class="section-block bold-text">
        <p>Constrataste a un jardinero y su trabajo se regar diariamente los rosales del jardin.</p>
        <p>¿C&oacute;mo vas a calificar su trabajo?</p>
      </div>
      <div class="section-block regular-text" align="left">
        <p><b>1 (NO CUMPLE): </b> En est&aacute; haciendo su trabajo. Probablemente va todos los d&iacute;as al jard&iacute;n, pero no est&aacute; regando los rosales.</p><br>
        <p><b>2 (CUMPLE PARCIALMENTE): </b> Riega los rosales un d&iacute;a si y un d&iacute;a no o tal vez los riega diario pero solo riega una parte de ellos. Esta haciendo su trabajo pero no como debe hacerlo.</p><br>
        <p><b>3 (CUMPLE 100% CON SU TRABAJO): </b> Todos los d&iacute;as llega a su hora y riega todos los rosales del jard&iacute;n. Est&aacute; su trabajo al 100%!! Adicionalmente su trabajo lo hace cantando? tiene la mejor de las actitudes? Perfecto! Est&aacute; cumpliendo con su trabajo y ¡s&uacute;per! que lo haga con gusto!!</p><br>
        <p><b>4 (EXCEDE EXPECTATIVAS): </b> Aparte de regar los rosales diariamente con la mejor de la actitudes y llegar a su hora, corta las hojas secas de cada rosal, poda las ramas que necesitan se podada, barre las hojas ca&iacute;das: est&aacute; haciendo actividades adicionales relacionadas con su trabajo que no son parte de &eacute;ste. Est&aacute; excediendo las expectativas y le ponemos un 4!</p><br>
        <p><b>5 (ES UN MODELO A SEGUIR): </b> Aparte de hacer su trabajo al 100%, exceder expectativas a ir mas all&aacute; de lo que se espera de &eacute;l, por voluntad propia apoya y riega los tulipanes, junta a los dem&aacute;s jardineros y les enseña t&eacute;cnicas de jardiner&iacute;a, fumiga el jard&iacute;n, etc. Es decir es un modelo a seguir para los dem&aacute; y sobre todo es reconocido por sus compañeros.</p><br>
      </div>
    </div>
  </div>

  <div class="col-xlg-3 col-lg-12  col-md-6 col-sm-6">
    <div ba-panel ba-panel-class="with-scroll more-text-widget" ba-panel-title="Raiting Anual">
      <div class="section-block light-text" align="left">
        <p>Al final del perido de evaluaci&oacute;n, cuando ya se realizaron todas las evaluaciones por parte de cada uno de nosotros y de los evaluadores anuales, de proyecto y 360º, se lleva a cabo la junta anual plenaria en donde se re&uacute;n Gerentes y Directores con el fin de revisar una a una las evaluaciones buscando homologar criterios de evaluaci&oacute;n y asegurando la objetividad en el proceso.</p>
        <p>En esta junta es donde se asignan los raitings anuales finales que conocer&aacute;s en tu feedback:</p>
      </div>
      <div class="section-block regular-text" align="left">
        <p><b>A: </b> Es un rol model para la organizaci&oacute;n por sus m&eacute;ritos durante el año y es una persona por todos los niveles de la organizaci&oacute;n.</p><br>
        <p><b>B: </b> Excede expectativas: es quien durante el año de forma constante fue mas all&aacute; de lo que su trabajo exige.</p><br>
        <p><b>C: </b>Cumple expectativas: es lo esperado por la organizaci&oacute;. Es aquella persona que hace muy bien su trabajo y cumple con todo lo que &eacute;ste implica.</p><br>
        <p><b>D: </b> Cumplo pacialmente: cumpli&oacute; parcialmente con lo esperado ya sea en aptitud o en actitud.</p><br>
        <p><b>E: </b> No cumple: el desempeño fue deficiente durante el año, no cumpli&oacute; en aptitudes y/o actitudes.</p><br>
      </div>
    </div>
  </div>

</div><br>